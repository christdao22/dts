                    ...
                    <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive"
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{!! $document->document_category_id != null? $document->document_category->category_name : "<b>Others: </b>" . $document->type !!}</td>
                                ...
                                <td>
                                    <div class="d-flex justify-content-end gap-1">
                                        <a class="ri ri-printer-fill btn btn-warning" data-bs-toggle="tooltip" data-bs-placement="top" title="Print"
                                            href="{{ route('document.pdf_view',$document->id ) }}" target="_blank">
                                        <a class="btn btn-info" data-bs-toggle="tooltip" data-bs-placement="top" title="Track"
                                            href="{{ route('web.find', 'query='.$document->document_code) }}"><i
                                            class="ri-route-line"></i></a>
                                        @if (!$document->documentTracking->is_received)
                                            <button class="btn btn-danger deleteBtn" data-bs-id={{ $document->id }}><i class="ri-delete-bin-line"></i>
                                                <form id="delete_form_{{ $document->id }}" action="{{ route('document.destroy', $document->id) }}"
                                                    method="POST" enctype="multipart/form-data">
                                                    @method('DELETE')
                                                    @csrf
                                                </form>
                                            </button>
                                            <a class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#myModal-{{ $document->id }}">EDIT</a>
                                        @endif
                                    ...
                                    <div class="modal" id="myModal-{{ $document->id }}">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                ...
                                                <form action="{{ route('document.updateEdit' ,$document->id) }}"
                                                    method="POST" enctype="multipart/form-data">
                                                    @method('PATCH')
                                                    @csrf
                                                    <div class="modal-body">
                                                        <div class="mb-3">
                                                            ...
                                                            <div class="mb-3">
                                                                ...
                                                                <input type="text" name="type_{{ $document->id }}" id="type_{{ $document->id }}" value="{{ $document->type }}"
                                                            ...
                                                            <div class="mb-3">
                                                                ...
                                                                <textarea id="" class="form-control" required
                                                                    name="name_of_client">{{ $document->name_of_client }}</textarea>
                                                            </div>
                                                            ...
                                                            <div class="mb-3">
                                                                <label class="form-label" for="terminal_id"><b>Recipient
                                                                    </b></label>
                                                                <select name="terminal_id" id="terminal_id" class="form-select" required>
                                                                    <option value="" disabled selected>Select recipient </option>
                                                                    @foreach ($terminals as $terminal)
                                                                    <option value="{{ $terminal->id }}"
                                                                        {{ old('terminal_id') == $terminal->id || $document->terminal->id == $terminal->id? 'selected':'' }}>
                                                                        {{ $terminal->terminal_name }}</option>
                                                                    @endforeach
                                                                </select>
                                                            </div>
            ...
            $('#is_check_by_dm').on('change', function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $('#terminal').empty().append('<option value="" disabled selected>Select recipient</option>')
                        $.each(data.terminal, function(index, item) {

                            $('#terminal').append(`<option value="${item.id}">${item.terminal_name}</option>`);
                        });
                    },
